...
df_product |> 
  mutate(
    unit_price = ifelse(is.na(unit_price), floor(mean(unit_price, na.rm = T)), unit_price),
    unit_cost = ifelse(is.na(unit_cost), floor(mean(unit_cost, na.rm = T)), unit_cost)
  ) |> 
  summarise(
...
df_product |> 
  mutate(
    unit_price = replace_na(floor(mean(unit_price, na.rm = T))),
    unit_cost = replace_na(floor(mean(unit_cost, na.rm = T)))
  )
...
df_product |> 
  mutate(
    unit_price = ifelse(is.na(unit_price), floor(median(unit_price, na.rm = T)), unit_price),
    unit_cost = ifelse(is.na(unit_cost), floor(median(unit_cost, na.rm = T)), unit_cost)
  ) |> 
  summarise(
    across(everything(),\(x) sum(is.na(x)))
  )  
...
df_customer |> 
  left_join(df_receipt) |> 
  mutate(
    year = year(ymd(sales_ymd))
  ) |> 
  group_by(customer_id) |> 
  summarise(
    total_amount = sum(amount),
    amount_2019 = sum(amount[year == 2019]),
    ratio_2019 = amount_2019 / total_amount
  ) |> 
  filter(
    ratio_2019 > 0
  ) |> 
...
df_customer |> 
  left_join(df_geocode |> rename(postal_cd = `﻿postal_cd`), by = "postal_cd") |> 
  group_by(customer_id) |> 
  summarise(
    postal_cd = first(postal_cd),
...
> 緯度（ラジアン）：\phi, \\
> 経度（ラジアン）：\lambda, \\
> 距離：L = 6371 \times \arccos(\sin \phi_1 \times \sin \phi_2 + \cos \phi_1 \times \cos \phi_2 \times \cos(\lambda_1 − \lambda_2))    
> $$
...
df_customer |> 
  ...
  summarise(
    postal_cd = first(postal_cd),
    application_store_cd = first(application_store_cd),
    home_longitude = mean(longitude),
    home_latitude = mean(latitude)
  ) |> 
  left_join(
    df_store |>
      select(
    ...
    join_by(application_store_cd == store_cd)
  ) |> 
  mutate(
    home_longitude_radian = home_longitude * pi / 180,
    home_latitude_radian = home_latitude * pi / 180,
    store_longitude_radian = store_longitude * pi / 180,
    store_latitude_radian = store_latitude * pi / 180,
    distance =
      6371 * acos(sin(home_latitude_radian) * sin(store_latitude_radian) + cos(home_latitude_radian) * cos(store_latitude_radian) * cos(home_longitude_radian - store_longitude_radian))
  ) |> 
  head(10)
...
df_nayose <-
  ...
  filter(
    amount == max_amount | max_amount |> is.na()
  ) |> 
  filter(
    customer_id == min(customer_id)
  ) |> 
  ungroup()

df_customer |> nrow() - df_nayose |> nrow()


# 以下でもまったく同じことが可能

df_nayose <- df_customer |> 
  left_join(df_receipt, by = "customer_id") |> 
  group_by(customer_id) |> 
  summarise(
  ...
  group_by(customer_name, postal_cd) |> 
  # amount が一番大きい人を残す（同率1位がいる場合は with_ties = TRUE で両方残る）
  slice_max(amount, with_ties = TRUE) |> 
  
  # 同率1位の中から、customer_id が一番小さい人を1人だけ残す
  slice_min(customer_id, n = 1, with_ties = FALSE) |> 
  ungroup()
...
df_customer |> n_distinct() - 
df_customer |> 
  left_join(df_nayose, join_by("customer_name", "postal_cd")) |> 
  select(all_of(colnames(df_customer |> rename(customer_id.x = customer_id))), togo_nayose_id = customer_id.y) |> 
  distinct(togo_nayose_id) |> 
  count()
...
df_customer_div <-
  ...
  filter(!is.na(amount)) |> 
  mutate(
    test = sample(c(0,1), n(), prob = c(0.8, 0.2), replace = T)
  )
...
df_receipt_div <-
  df_receipt |> 
  mutate(month = ymd(sales_ymd) |> floor_date("month")) |> 
  group_by(month) |>
  summarise(monthly_amount = sum(amount)) |> 
  mutate(
    test1 = sample(c(rep(0,16), rep(1,12), rep(2,6))),
    test2 = sample(c(rep(0,16), rep(1,12), rep(2,6))),
    test3 = sample(c(rep(0,16), rep(1,12), rep(2,6)))
  )
...
# 時系列分析用というのはそういうことじゃなくて、連続する18か月を取らないといけないらしい。
# まあこんなんでいいんじゃないですかね。
...
df_receipt_div <-
  ...
  summarise(monthly_amount = sum(amount)) |> 
  mutate(
    test1 = c(rep(0,12), rep(1,6), rep(NA,16)),
    test2 = c(rep(NA,8), rep(0,12), rep(1,6), rep(NA,8)),
    test3 = c(rep(NA,16), rep(0,12), rep(1,6))
  )
